...
if [ "$GENERATE_AUDIO" = true ]; then
  git add "$ARTICLE_FILE" "$BLOG_HTML" feed.xml "$AUDIO_DIR/$ARTICLE_BASE.mp3" podcast.html article-titles.txt
  git commit -m "📝 发布文章: $ARTICLE_BASE (带语音)"
else
  git add "$ARTICLE_FILE" "$BLOG_HTML" feed.xml podcast.html article-titles.txt
